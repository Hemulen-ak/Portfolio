...
(function init() {
    ...
    function buildTemplate(type, style) {
        ...
        const brandingHtmlLeft = calendarMode === 'organization' ? `
            <div style="display:flex;align-items:center;gap:12px;position:absolute;top:20px;left:25px;z-index:30;background:rgba(255,255,255,0.95);padding:10px 15px;border-radius:12px;box-shadow:0 8px 25px rgba(0,0,0,0.12);max-width:280px;">
                ${orgLogoUrl ? `<img src="${orgLogoUrl}" style="height:45px;width:auto;max-width:100px;object-fit:contain;">` : ''}
                <div style="display:flex;flex-direction:column;justify-content:center;">
                    <span style="font-size:1.05rem;color:#111;font-weight:800;line-height:1.1;letter-spacing:-0.01em;">${orgName || 'Your Organization'}</span>
                </div>
